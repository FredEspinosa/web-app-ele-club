...
import { Pagination, Autoplay } from "swiper/modules"; // Importar el módulo de paginación
...
const CarruselPerfilUsuario = ({setNombres, setEdad, setPronombres, userPhotos, infoPerfil, autoPlaySlide, autoplayInterval}) => {

  const modules = [Pagination];   // Agregamos el módulo de paginación
  if (autoPlaySlide) {
    modules.push(Autoplay); // Agregar Autoplay solo si está habilitado
  }

  return (
    <div className="club_carrusel_perfil carousel-container">
      <Swiper
        modules={modules} // Pasar los módulos dinámicamente 
        ...
        autoplay={
          autoPlaySlide
            ? {
                delay: autoplayInterval, // Configura el intervalo (en milisegundos)
                disableOnInteraction: false, // No detiene autoplay al interactuar
              }
            : undefined // No pasar la propiedad autoplay si no está habilitada
        }
      >
